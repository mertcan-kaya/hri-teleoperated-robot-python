...
if __name__ == "__main__":
    ...
    marker_pos_ini = [-0.133,-0.999,1.498]
    ...
    while keep_running:
        ...
        Pos_TCP = state.actual_TCP_pose[0:3]
        ...
        if state.runtime_state == 2: # Play button is pressed
            ...
            if state.output_int_register_0 == 0: # Continue button is pressed
                ...
                #print('play')
                idx = idx + 1
                if ini_flag == 0: # Markers are not at the starting position yet
                    print(marker_pos_try)
                    if LA.norm(marker_pos-marker_pos_try) < 0.1:
                        ini_flag = 1
                        Pos_TCP_ini = state.actual_TCP_pose[0:3]
                        print('GO!')
                else: # Market data is now sending to the robot
                    if LA.norm(marker_pos-marker_pos_try) < 0.1:
                        marker_pos = marker_pos_try
                        # err_coef = err_coef_trk

                    Pos_TCP = state.actual_TCP_pose[0:3]
                    Pos_TCP_zero = np.array(Pos_TCP)-np.array(Pos_TCP_ini)

                    #print(Pos_TCP_zero)

                    if Pos_TCP_zero[0] > max_x:
                        max_x = Pos_TCP_zero[0]

                    if Pos_TCP_zero[0] < min_x:
                        min_x = Pos_TCP_zero[0]

                    if Pos_TCP_zero[1] > max_y:
                        max_y = Pos_TCP_zero[1]

                    if Pos_TCP_zero[1] < min_y:
                        min_y = Pos_TCP_zero[1]

                    if Pos_TCP_zero[2] > max_z:
                        max_z = Pos_TCP_zero[2]

                    if Pos_TCP_zero[2] < min_z:
                        min_z = Pos_TCP_zero[2]
            ...
            if mounting_seq == SHLDRM:
                # Pos_TCP: up = -x (0,-0.325), front = -y (0,-0.12), right = z (-0.39,0.02)
                ...
                if Pos_z < 0.45 and Pos_x > -0.06:
                    if Pos_TCP[0] > -0.05 and Pos_TCP[2] >= -0.2:
                        Pos_z = 0.45
                    else:
                        Pos_x = -0.06
                ...
                # Pos_TCP: up = z (0,0.325), front = -y (0,-0.12), right = -x (0.047,-0.467)
                ...
                if Pos_x < 0.11 and Pos_z < 0.18:
                    if Pos_TCP[2] < 0.2 and Pos_TCP[0] < -0.2:
                        Pos_z = Pos_ini[0] - 0.1
                    else:
                        Pos_x = Pos_ini[0] - 0.3
